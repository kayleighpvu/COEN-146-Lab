...
* Description: This program is would find the least costful path between each router
* 			   given the distances between each of them.  Also will have to mark 
*			   or "add" the distance if a router is going through another to get to its
*			   target router.
*/
...
void print_costs (void)
{
	// print costs
	int i, j;
	//iterate through all of matrix using nested for loop
	for (i = 0; i < N; i++) //globally defined N = 4
	{
		for (j = 0; j < N; j++)	
		{
			printf("%d ", costs[i][j]);
...
void * run_link_state (void *arg)
{
	...
	while (1)
	{
		...
		for (i = 1; i < N; i++)
		{	
			//default initialize min and spot (Dijkstra's algorithm) 
			min = INFINITE;
			spot = -1;
			...
			for (k = 0; k < N; k++)
			{
				...
				{
					//update the new minimum value and spot
					min = distances[k]; 
					spot = k;
				}
			}
			...
			//mark that the spot has been visited
			taken[spot] = 1; 
			...
			for (j = 0; j < N; j++)
			{
				if (taken[j] == 0)	//if router is not visited yet
				{
					//to not change values in matrix, just want to find min
					pthread_mutex_lock (&lock);
