...
* Description: Similar to Lab 5, the server side will wait for the client to call
* the server for connection and start sending it's packets.  If the server has
* received the packet, it will send and ACK back to the client for them to send the 
* next packet.  If the server does not receive the packet or something wrong has
* happened, the server will send a message that the packet was not received or the client's 
* timer will run out and will have to retransmit the packet.
*/
...
//Print packet: data, sequence/acknowledgement number, and checksum
